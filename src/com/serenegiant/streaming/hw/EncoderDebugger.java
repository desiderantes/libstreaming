...
package com.serenegiant.streaming.hw;
...
public class EncoderDebugger {
	private final static String TAG = EncoderDebugger.class.getSimpleName();
	...
	public synchronized static void asyncDebug(@NonNull final Bundle settings, final int width, final int height) {
		new Thread(new Runnable() {
			@Override
			public void run() {
				try {
					debug(settings, width, height);
				} catch (Exception e) {
					Log.w(TAG, e);
				}
			}
	...
	public synchronized static EncoderDebugger debug(@NonNull final Bundle settings, int width, int height) {
		final EncoderDebugger debugger = new EncoderDebugger(settings, width, height);
		debugger.debug();
		return debugger;
	}
	...
	private EncoderDebugger(@NonNull final Bundle settings, int width, int height) {
		mSettings = settings;
		mWidth = width;
		mHeight = height;
		mSize = width*height;
	...
	private void debug() {
		...
		if (!checkTestNeeded()) {
			...
			boolean success = mSettings.getBoolean(PREF_PREFIX+resolution+"success",false);
			if (!success) {
				throw new RuntimeException("Phone not supported with this resolution ("+mWidth+"x"+mHeight+")");
			}

			mNV21.setSize(mWidth, mHeight);
			mNV21.setSliceHeigth(mSettings.getInt(PREF_PREFIX+resolution+"sliceHeight", 0));
			mNV21.setStride(mSettings.getInt(PREF_PREFIX+resolution+"stride", 0));
			mNV21.setYPadding(mSettings.getInt(PREF_PREFIX+resolution+"padding", 0));
			mNV21.setPlanar(mSettings.getBoolean(PREF_PREFIX+resolution+"planar", false));
			mNV21.setColorPanesReversed(mSettings.getBoolean(PREF_PREFIX+resolution+"reversed", false));
			mEncoderName = mSettings.getString(PREF_PREFIX+resolution+"encoderName", "");
			mEncoderColorFormat = mSettings.getInt(PREF_PREFIX+resolution+"colorFormat", 0);
			mB64PPS = mSettings.getString(PREF_PREFIX+resolution+"pps", "");
			mB64SPS = mSettings.getString(PREF_PREFIX+resolution+"sps", "");
	...
	private boolean checkTestNeeded() {
		...
		// Forces the test
		if (DEBUG || mSettings ==null) return true;

		// If the sdk has changed on the phone, or the version of the test
		// it has to be run again
		if (mSettings.containsKey(PREF_PREFIX+resolution+"lastSdk")) {
			int lastSdk = mSettings.getInt(PREF_PREFIX+resolution+"lastSdk", 0);
			int lastVersion = mSettings.getInt(PREF_PREFIX+resolution+"lastVersion", 0);
	...
	private void saveTestResult(boolean success) {
		...
		mSettings.putBoolean(PREF_PREFIX+resolution+"success", success);

		if (success) {
			mSettings.putInt(PREF_PREFIX+resolution+"lastSdk", Build.VERSION.SDK_INT);
			mSettings.putInt(PREF_PREFIX+resolution+"lastVersion", VERSION);
			mSettings.putInt(PREF_PREFIX+resolution+"sliceHeight", mNV21.getSliceHeigth());
			mSettings.putInt(PREF_PREFIX+resolution+"stride", mNV21.getStride());
			mSettings.putInt(PREF_PREFIX+resolution+"padding", mNV21.getYPadding());
			mSettings.putBoolean(PREF_PREFIX+resolution+"planar", mNV21.getPlanar());
			mSettings.putBoolean(PREF_PREFIX+resolution+"reversed", mNV21.getUVPanesReversed());
			mSettings.putString(PREF_PREFIX+resolution+"encoderName", mEncoderName);
			mSettings.putInt(PREF_PREFIX+resolution+"colorFormat", mEncoderColorFormat);
			mSettings.putString(PREF_PREFIX+resolution+"encoderName", mEncoderName);
			mSettings.putString(PREF_PREFIX+resolution+"pps", mB64PPS);
			mSettings.putString(PREF_PREFIX+resolution+"sps", mB64SPS);
		}
